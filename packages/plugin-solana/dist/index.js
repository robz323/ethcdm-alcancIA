...
import { settings as settings3, elizaLogger as elizaLogger7 } from "@elizaos/core";
var solAddress = settings3.SOL_ADDRESS;
var SLIPPAGE = settings3.SLIPPAGE;
var connection2 = new Connection6(
  settings3.SOLANA_RPC_URL || "https://api.mainnet-beta.solana.com"
);
...
async function swapToken(connection3, walletPublicKey, inputTokenCA, outputTokenCA, amount) {
  try {
    ...
    if (!quoteData || quoteData.error) {
      elizaLogger8.error("Quote error:", quoteData);
      throw new Error(
        `Failed to get quote: ${quoteData?.error || "Unknown error"}`
      );
    }
    elizaLogger8.log("Quote received:", quoteData);
    ...
    elizaLogger8.log("Requesting swap with body:", swapRequestBody);
    ...
    if (!swapData || !swapData.swapTransaction) {
      elizaLogger8.error("Swap error:", swapData);
      throw new Error(
        `Failed to get swap transaction: ${swapData?.error || "No swap transaction returned"}`
      );
    }
    elizaLogger8.log("Swap transaction received");
    return swapData;
  } catch (error) {
    elizaLogger8.error("Error in swapToken:", error);
    throw error;
  }
}
...
async function getTokenFromWallet(runtime, tokenSymbol) {
  try {
    ...
    elizaLogger8.error("Error checking token in wallet:", error);
    return null;
  }
}
var executeSwap = {
  ...
  validate: async (runtime, message) => {
    elizaLogger8.log("Message:", message);
    return true;
  },
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    elizaLogger8.log("Response:", response);
    ...
    if (!response.amount) {
      elizaLogger8.log("No amount provided, skipping swap");
      const responseMsg = {
    ...
    if (!response.amount) {
      elizaLogger8.log("Amount is not a number, skipping swap");
      const responseMsg = {
    ...
    try {
      ...
      elizaLogger8.log("Wallet Public Key:", walletPublicKey);
      elizaLogger8.log("inputTokenSymbol:", response.inputTokenCA);
      elizaLogger8.log("outputTokenSymbol:", response.outputTokenCA);
      elizaLogger8.log("amount:", response.amount);
      ...
      elizaLogger8.log("Deserializing transaction...");
      const transactionBuf = Buffer.from(
        swapResult.swapTransaction,
        "base64"
      );
      const transaction = VersionedTransaction2.deserialize(transactionBuf);
      elizaLogger8.log("Preparing to sign transaction...");
      elizaLogger8.log("Creating keypair...");
      ...
      elizaLogger8.log("Signing transaction...");
      transaction.sign([keypair]);
      elizaLogger8.log("Sending transaction...");
      ...
      elizaLogger8.log("Transaction sent:", txid);
      ...
      elizaLogger8.log("Swap completed successfully!");
      elizaLogger8.log(`Transaction ID: ${txid}`);
      const responseMsg = {
      ...
      elizaLogger8.error("Error during token swap:", error);
      return false;
    }
  },
  examples: [
...
function isCreateAndBuyContent(runtime, content) {
  elizaLogger9.log("Content for create & buy", content);
...
var createAndBuyToken = async ({
  ...
  elizaLogger9.log("Create Results: ", createResults);
  if (createResults.success) {
    ...
    if (amount === null) {
      ...
      elizaLogger9.log(`${deployer.publicKey.toBase58()}:`, amount);
    }
    return {
      success: true,
    ...
    elizaLogger9.log("Create and Buy failed");
    return {
      success: false,
      ca: mint.publicKey.toBase58(),
...
var pumpfun_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger9.log("Starting CREATE_AND_BUY_TOKEN handler...");
    if (!state) {
      state = await runtime.composeState(message);
    } else {
      state = await runtime.updateRecentMessageState(state);
    ...
    elizaLogger9.log(`Base64 data saved to: ${outputPath}`);
    ...
    try {
      ...
      const connection3 = new Connection8(settings5.SOLANA_RPC_URL, {
        commitment: "confirmed",
        confirmTransactionInitialTimeout: 5e5,
        // 120 seconds
        wsEndpoint: settings5.SOLANA_RPC_URL.replace("https", "wss")
      });
      ...
      if (!createAndBuyConfirmation) {
        elizaLogger9.log("Create and buy token canceled by user");
        return false;
      }
      const lamports = Math.floor(Number(buyAmountSol) * 1e9);
      elizaLogger9.log("Executing create and buy transaction...");
...
Attempted mint address: ${result.ca}`,
      ...
      elizaLogger9.log(successMessage);
...
// src/actions/fomo.ts
import { generateImage as generateImage2, elizaLogger as elizaLogger10 } from "@elizaos/core";
import {
...
function isCreateAndBuyContentForFomo(content) {
  elizaLogger10.log("Content for create & buy", content);
...
var createAndBuyToken2 = async ({
  ...
  elizaLogger10.log("Transaction sent:", txid);
  const confirmation = await connection3.confirmTransaction(
    {
      signature: txid,
      blockhash,
  ...
  if (!confirmation.value.err) {
    ...
    if (amount === null) {
      ...
      elizaLogger10.log(`${deployer.publicKey.toBase58()}:`, amount);
    }
    return {
      success: true,
      ca: mint.publicKey.toBase58(),
    ...
    elizaLogger10.log("Create and Buy failed");
    return {
      success: false,
      ca: mint.publicKey.toBase58(),
...
var fomo_default = {
  ...
  description: "Create a new token and buy a specified amount using SOL. Requires deployer private key, token metadata, buy amount in SOL, priority fee, and allowOffCurve flag.",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger10.log("Starting CREATE_AND_BUY_TOKEN handler...");
    if (!state) {
      state = await runtime.composeState(message);
    } else {
      state = await runtime.updateRecentMessageState(state);
    ...
    try {
      ...
      const connection3 = new Connection9(settings6.SOLANA_RPC_URL, {
        commitment: "confirmed",
        confirmTransactionInitialTimeout: 5e5,
        // 120 seconds
        wsEndpoint: settings6.SOLANA_RPC_URL.replace("https", "wss")
      });
      const sdk = new Fomo(connection3, "devnet", deployerKeypair);
      const createAndBuyConfirmation = await promptConfirmation2();
      if (!createAndBuyConfirmation) {
        elizaLogger10.log("Create and buy token canceled by user");
        return false;
      }
      const lamports = Math.floor(Number(buyAmountSol) * 1e9);
      elizaLogger10.log("Executing create and buy transaction...");
...
Attempted mint address: ${result.ca}`,
      ...
      elizaLogger10.log(successMessage);
      return result.success;
    } catch (error) {
      if (callback) {
        callback({
...
var executeSwapForDAO = {
  ...
  handler: async (runtime, message) => {
    ...
    try {
      ...
      elizaLogger11.log("Swap Quote:", quoteData);
      const confirmSwap = await promptConfirmation3();
      if (!confirmSwap) {
        elizaLogger11.log("Swap canceled by user");
        return false;
      }
      ...
      elizaLogger11.log("DAO Swap completed successfully!");
      elizaLogger11.log(`Transaction ID: ${txid}`);
      return true;
    } catch (error) {
      elizaLogger11.error("Error during DAO token swap:", error);
      return false;
    }
  },
  examples: [
...
import { elizaLogger as elizaLogger12, settings as settings7 } from "@elizaos/core";
import {
...
function isTransferContent(runtime, content) {
  elizaLogger12.log("Content for transfer", content);
...
var transfer_default = {
  ...
  validate: async (runtime, message) => {
    elizaLogger12.log("Validating transfer from user:", message.userId);
    return false;
  },
  description: "Transfer tokens from the agent's wallet to another address",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger12.log("Starting SEND_TOKEN handler...");
    if (!state) {
      state = await runtime.composeState(message);
    } else {
      state = await runtime.updateRecentMessageState(state);
    ...
    if (!isTransferContent(runtime, content)) {
      elizaLogger12.error("Invalid content for TRANSFER_TOKEN action.");
      if (callback) {
        callback({
    ...
    try {
      ...
      const connection3 = new Connection11(settings7.SOLANA_RPC_URL);
      ...
      const signature = await connection3.sendTransaction(transaction);
      elizaLogger12.log("Transfer successful:", signature);
      if (callback) {
        callback({
...
Transaction: ${signature}`,
      ...
      elizaLogger12.error("Error during token transfer:", error);
      if (callback) {
        callback({
...
import { getAccount, getAssociatedTokenAddress as getAssociatedTokenAddress3 } from "@solana/spl-token";
...
async function getTokenBalance(connection3, walletPublicKey, tokenMintAddress) {
  const tokenAccountAddress = await getAssociatedTokenAddress3(
    tokenMintAddress,
    walletPublicKey
  );
  try {
...
async function getTokenBalances(connection3, walletPublicKey) {
  ...
  const tokenMintAddresses = [
    new PublicKey11("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"),
    // USDC
    new PublicKey11("So11111111111111111111111111111111111111112")
    // SOL
