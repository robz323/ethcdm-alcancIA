import { ComputeBudgetInstruction, ComputeBudgetProgram, Message, Transaction, TransactionMessage, VersionedTransaction, } from '@solana/web3.js';
import assert from 'assert';
import bs58 from 'bs58';
import { backOff } from 'exponential-backoff';
import { sleep, waitMS } from '../time';
import { filterNullLike, isNullLike, makeBatches, settleAllWithTimeout, } from '../utils';
...
export const castTxResponseJSON = (tx) => {
    return {
        ...tx,
        transaction: {
            ...tx.transaction,
...
export const castTxResponse = (tx) => {
    return {
        ...tx,
        transaction: {
            ...tx.transaction,
...
export const castMessageJSON = (msg) => {
    return {
        ...msg,
...
export class RetryTxSender {
    ...
    async send(tx) {
        ...
        try {
            ...
            this.logger?.info(`🚀 [${this.txSig.substring(0, 5)}] tx sent to MAIN connection`);
        ...
        (async () => {
            while (!this.done && this._getTimestamp() - startTime < this.timeout) {
                this.logger?.info(`🔁 [${this.txSig?.substring(0, 5)}] begin new retry loop (sleeping for ${this.retrySleep / 1000}s)`);
                await this._sleep();
                if (!this.done) {
                    this.connection
    ...
    async _confirmTransaction(txSig, lastValidBlockHeight, opts) {
        this.logger?.info(`⏳ [${txSig.substring(0, 5)}] begin trying to confirm tx`);
        ...
        const promises = connections
            .map((connection, i) => {
            let subscriptionId;
            const pollPromise = backOff(async () => {
        ...
        if (response === null) {
            const errMsg = `❌ [${txSig.substring(0, 5)}] NOT confirmed in ${duration.toFixed(2)}sec`;
            this.logger?.error(errMsg);
            throw new Error(errMsg);
        }
        if (response.value.err) {
            this.logger?.warn(`⚠️ [${txSig.substring(0, 5)}] confirmed AS FAILED TX in ${duration.toFixed(2)}sec`);
        }
        else {
            this.logger?.info(`✅ [${txSig.substring(0, 5)}] confirmed in ${duration.toFixed(2)}sec`);
        }
        return response;
    }
    _getTimestamp() {
    ...
    async _outdatedBlockHeightPromise(lastValidBlockHeight) {
        ...
        if (currentHeight > lastValidBlockHeight) {
            this.logger?.error(`❌ [${this.txSig?.substring(0, 5)}] current height ${currentHeight - lastValidBlockHeight} blocks > lastValidBlockHeight, aborting`);
        }
        return null;
    }
    ...
    _sendToAdditionalConnections(rawTx) {
        ...
        this.logger?.info(`💥 [${this.txSig?.substring(0, 5)}] tx sent to ${this.additionalConnections.length} ADDITIONAL connections`);
    }
...
const maybeFetchBlockhash = async (maybeBlockhash) => {
    ...
    if (maybeBlockhash.type === 'blockhash') {
        ...
        const { blockhash: blockhash_, lastValidBlockHeight: lastValidBlockHeight_, } = await getLatestBlockhashMultConns(maybeBlockhash.args);
...
export const buildTx = async ({ feePayer, instructions, additionalSigners, maybeBlockhash, }) => {
...
export const buildTxV0 = async ({ feePayer, instructions, additionalSigners, addressLookupTableAccs, maybeBlockhash, }) => {
    if (!instructions.length) {
        throw new Error('must pass at least one instruction');
    }
    const { blockhash, lastValidBlockHeight } = await maybeFetchBlockhash(maybeBlockhash);
...
export const buildTxsLegacyV0 = async ({ feePayer, instructions, additionalSigners, addressLookupTableAccs, maybeBlockhash, }) => {
    if (!instructions.length) {
        throw new Error('must pass at least one instruction');
    }
    const { blockhash, lastValidBlockHeight } = await maybeFetchBlockhash(maybeBlockhash);
    ...
    const txV0 = new VersionedTransaction(msg.compileToV0Message(addressLookupTableAccs));
    if (additionalSigners) {
...
export const getLatestBlockhashMultConns = async ({ connections, commitment = 'confirmed', maxRetries = 5, startTimeoutMs = 100, maxTimeoutMs = 2000, }) => {
    let retries = 0;
    ...
    while (blockhashes.length < 1 && retries < maxRetries) {
        ...
        blockhashes = await settleAllWithTimeout(connections.map((c) => c.getLatestBlockhashAndContext(commitment)), timeoutMs);
        retries++;
 ...
 */
export const confirmTransactionMultConns = async ({ conns, sig, timeoutMs = 60 * 1000, maxDelayMs = 10 * 1000, startingDelayMs = 3 * 1000, numOfAttempts = 7, }) => {
    return await Promise.race([
        ...
        //   })
        // ),
        ...conns.map(async (c) => backOff(async () => {
...
export const getLatestBlockHeight = async ({ connections, commitment = 'confirmed', maxRetries = 5, startTimeoutMs = 100, maxTimeoutMs = 2000, }) => {
    let retries = 0;
    let timeoutMs = startTimeoutMs;
    let heights = [];
    while (heights.length < 1 && retries < maxRetries) {
        ...
        //as per advice here https://jstarry.notion.site/Transaction-confirmation-d5b8f4e09b9c4a70a1f263f82307d7ce
        heights = await settleAllWithTimeout(connections.map((c) => c.getBlockHeight(commitment)), timeoutMs);
        retries++;
        timeoutMs = Math.min(timeoutMs * 2, maxTimeoutMs);
    }
    if (!heights.length) {
...
export const serializeAnyVersionTx = (tx, verifySignatures = false) => {
...
export const legacyToV0Tx = (legacy) => {
...
export const getAccountWaitSlot = async ({ conn, slot, pubkey, beforeHook, retries = 5, }) => {
    let curSlot;
    let account;
    while (true) {
        ({
...
export const getMultipleAccountsWaitSlot = async ({ conn, slot, pubkeys, beforeHook, retries = 5, }) => {
    return (await Promise.all(makeBatches(pubkeys, 100).map(async (batch, batchIdx) => {
        let curSlot;
        let accounts;
        while (true) {
            ({
...
export const getAccountKeys = (tx) => {
    return [
...
export const convertTxToLegacy = (tx) => {
    ...
    const legacyMsg = new Message({
        ...
        instructions: v0msg.compiledInstructions.map((i) => {
            ...
            return {
                ...
                data: 'data' in i.data
                    ? bs58.encode(Uint8Array.from(i.data.data))
                    : bs58.encode(i.data),
            };
        }),
...
export const getTransactionConvertedToLegacy = async (conn, sig, commitment = 'confirmed') => {
...
export const prependComputeIxs = (ixs, compute, priorityMicroLamports) => {
    const out = [...ixs];
    if (compute &&
        !ixs.some((ix) => ix.programId.equals(ComputeBudgetProgram.programId) &&
