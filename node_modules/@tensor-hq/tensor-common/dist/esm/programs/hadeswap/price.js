...
export const calculateNextSpotPrice = ({ orderType, spotPrice, delta, bondingCurveType, counter, }) => {
    if (bondingCurveType === HadeswapBondingCurveType.Linear) {
        let current_price = spotPrice; // 1
        const targetCounter = counter + (orderType === HadeswapOrderType.Buy ? 1 : -1);
        if (targetCounter >= 0) {
            // 0
        ...
        const newCounter = orderType === HadeswapOrderType.Buy ? counter + 1 : counter - 1;
        ...
        const counterUpdated = orderType === HadeswapOrderType.Buy ? counter : counter - 1;
...
export const computeHswapTakerPrice = ({ takerSide, config, extraNFTsSelected, }) => {
    const price = calculateNextSpotPrice({
        orderType: takerSide === 'Buy' ? HadeswapOrderType.Buy : HadeswapOrderType.Sell,
