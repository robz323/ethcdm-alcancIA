...
import { AUTH_PROGRAM_ID, TMETA_PROGRAM_ID, fetchMetadataByMint, prepPnftAccounts, } from '../../metaplex';
import { buildTx, getOrCreateAtaForMint } from '../../solana_contrib';
import { BADGER_PROGRAM_ID, SOLANART_ESCROW_OWNER_ACCT, SOLANART_FEE_ACCT, SOLANART_PROGRAM_ID, findBadgerPda, findDataEscrowPda, findRoyaltiesPda, } from './shared';
export const makeSolanartBuyTx = async ({ connections, buyer, seller, tokenMint, priceLamports, blockhash, }) => {
    const connection = connections[0];
    const instructions = [];
    const buyerAcc = new PublicKey(buyer);
    const sellerAcc = new PublicKey(seller);
    const mintAcc = new PublicKey(tokenMint);
    const { tokenAccount: targetTokenAccount, instructions: tokenInstr } = await getOrCreateAtaForMint({
        connection,
        owner: buyerAcc,
        mint: mintAcc,
    });
    ...
    if (metadata.tokenStandard === TokenStandard.ProgrammableNonFungible) {
        const { ruleSet, nftEditionPda, ownerTokenRecordPda, destTokenRecordPda } = await prepPnftAccounts({
            connection,
