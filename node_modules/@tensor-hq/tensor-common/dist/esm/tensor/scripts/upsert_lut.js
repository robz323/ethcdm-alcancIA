...
(async () => {
    ...
    const lookupTableAddress = cluster === 'mainnet-beta' ? TENSOR_LUT_ADDR : TENSOR_LUT_DEVNET_ADDR;
    const conn = new Connection(clusterApiUrl(cluster), 'confirmed');
    const payer = Keypair.fromSecretKey(Buffer.from(JSON.parse(readFileSync(kpFile).toString())));
    if (lutMode === 'create') {
        await createLUT({
            payer,
            conn,
