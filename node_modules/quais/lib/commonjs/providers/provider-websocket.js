...
class WebSocketProvider extends provider_socket_js_1.SocketProvider {
    ...
    initWebSocket(websocket, shard, port) {
        ...
        websocket.onopen = async () => {
            try {
                ...
                try {
                    const zone = (0, index_js_1.toZone)(shard);
        ...
        websocket.onclose = () => {
            setTimeout(() => {
                ...
                const shardSuffix = this._getOption('usePathing') ? `/${(0, shards_js_1.fromShard)(shard, 'nickname')}` : `:${port}`;
    ...
    createWebSocket = (baseUrl, suffix) => {
        const tempWs = new ws_js_1.WebSocket(`${baseUrl}${suffix}`);
        return tempWs;
        // wait 2 minutes
    };
    /**
    ...
    async initialize(urls) {
        ...
        this._urlMap.clear();
        try {
            const primeSuffix = this._getOption('usePathing') ? `/${(0, shards_js_1.fromShard)(index_js_1.Shard.Prime, 'nickname')}` : ':8001';
            const initShardWebSockets = async (baseUrl) => {
                const shards = await this._getRunningLocations(index_js_1.Shard.Prime, true);
                await Promise.all(shards.map(async (shard) => {
                    const port = 8200 + 20 * shard[0] + shard[1];
                    const shardEnum = (0, index_js_1.toShard)(`0x${shard[0].toString(16)}${shard[1].toString(16)}`);
                    const shardSuffix = this._getOption('usePathing')
                        ? `/${(0, shards_js_1.fromShard)(shardEnum, 'nickname')}`
                        : `:${port}`;
            ...
            if (Array.isArray(urls)) {
                for (const url of urls) {
                    ...
                    this.initWebSocket(primeWebsocket, index_js_1.Shard.Prime, 8001);
                    this.#websockets.push(primeWebsocket);
                    this._urlMap.set(index_js_1.Shard.Prime, primeWebsocket);
                    await this.waitShardReady(index_js_1.Shard.Prime);
