                ...
                return param.walkAsync(args[index], (type, value) => {
                    if (type === 'address') {
                        if (Array.isArray(value)) {
                            return Promise.all(value.map((v) => (0, index_js_2.resolveAddress)(v)));
                        }
...
async function copyOverrides(arg, allowed) {
    ...
    (0, index_js_3.assertArgument)(!('to' in overrides) || overrides.to == null || (allowed || []).indexOf('to') >= 0, 'cannot override to', 'overrides.to', overrides);
    (0, index_js_3.assertArgument)(!('data' in overrides) || overrides.data == null || (allowed || []).indexOf('data') >= 0, 'cannot override data', 'overrides.data', overrides);
    // Resolve any from
...
function buildWrappedFallback(contract) {
    ...
    const populateTransaction = async function (overrides) {
        ...
        (0, index_js_2.validateAddress)(tx.to);
        if (tx.from) {
            tx.from = await (0, index_js_2.resolveAddress)(tx.from);
            (0, index_js_2.validateAddress)(tx.from);
        }
...
function buildWrappedMethod(contract, key) {
    ...
    const populateTransaction = async function (...args) {
        ...
        const resolvedArgs = await resolveArgs(contract.runner, fragment.inputs, args);
        return await (0, index_js_3.resolveProperties)({
            to: contract.getAddress(),
            from: args.pop()?.from,
            data: contract.interface.encodeFunctionData(fragment, resolvedArgs),
        });
    ...
    const send = async function (...args) {
        ...
        if (!pop.from && 'address' in runner && typeof runner.address === 'string') {
            pop.from = await (0, index_js_2.resolveAddress)(runner.address);
        }
    ...
    const staticCallResult = async function (...args) {
        ...
        if (!tx.from && 'address' in runner && typeof runner.address === 'string') {
            tx.from = await (0, index_js_2.resolveAddress)(runner.address);
        }
        let result = '0x';
        try {
...
async function getSub(contract, operation, event) {
    ...
    if (!sub) {
        ...
        const zone = (0, index_js_3.getZoneForAddress)(await (0, index_js_2.resolveAddress)(address));
        let starting = [];
        const start = () => {
            if (starting.length) {
                return;
...
class BaseContract {
    ...
    async queryFilter(event, fromBlock, toBlock) {
        ...
        const zone = (0, index_js_3.getZoneForAddress)(address);
        const filter = { address, topics, fromBlock, toBlock, nodeLocation: (0, shards_js_1.getNodeLocationFromZone)(zone) };
        const provider = getProvider(this.runner);
