...
export const areFloatsEqual = (num1, num2, epsilon = 1e-10) => Math.abs(num1 - num2) < epsilon;
...
function testToMantissaAndExponent() {
    assert(toMantissaAndExponent(3).priceExponent == -8, `Unexpected exponent ${toMantissaAndExponent(3).priceExponent}`);
    assert(toMantissaAndExponent(3).priceMantissa == 300_000_000, `Unexpected manitssa ${toMantissaAndExponent(3).priceMantissa}`);
}
describe('Utils test', () => {
    it('Utils', async () => {
        await testUtils();
