...
async function testBatchUpdate() {
    ...
    await batchUpdate(connection, payerKeypair, marketAddress, 5, 5, false, OrderType.Limit, 0);
...
async function testBatchUpdateWithGlobal() {
    const connection = new Connection('http://127.0.0.1:8899', 'confirmed');
    const payerKeypair = Keypair.generate();
    const marketAddress = await createMarket(connection, payerKeypair);
    const market = await Market.loadFromAddress({
    ...
    await depositGlobal(connection, payerKeypair, market.quoteMint(), 10, payerKeypair);
    await batchUpdate(connection, payerKeypair, marketAddress, 1, 1, true, OrderType.Global, 0);
    await market.reload(connection);
    market.prettyPrint();
    assert(market.bids().length == 1, 'batch update did not work for global');
    assert(Number(market.bids()[0].numBaseTokens) == 1, 'ask top of book wrong size');
    assert(market.bids()[0].tokenPrice == 1, `ask top of book wrong price ${market.bids()[0].tokenPrice}`);
    assert(market.asks().length == 0, 'place asks did not work');
}
async function batchUpdate(connection, payerKeypair, marketAddress, numBaseTokens, tokenPrice, isBid, orderType, clientOrderId, lastValidSlot = 0) {
