...
class EvmWormholeCore {
    ...
    constructor(network, chain, provider, contracts) {
        ...
        this.coreIface = index_js_1.ethers_contracts.Implementation__factory.createInterface();
        const address = this.contracts.coreBridge;
        if (!address)
            throw new Error('Core bridge address not found');
        this.coreAddress = address;
        this.core = index_js_1.ethers_contracts.Implementation__factory.connect(address, provider);
    }
    async getMessageFee() {
    ...
    async *verifyMessage(sender, vaa) {
        const senderAddr = new sdk_evm_1.EvmAddress(sender).toString();
        const txReq = await this.core.parseAndVerifyVM.populateTransaction((0, sdk_connect_1.serialize)(vaa));
        yield this.createUnsignedTx((0, sdk_evm_1.addFrom)(txReq, senderAddr), 'WormholeCore.verifyMessage');
    }
    ...
    async parseMessages(txid) {
        ...
        return receipt.logs
            ...
            return (0, sdk_connect_1.createVAA)('Uint8Array', {
                ...
                signatures: [],
                payload: sdk_connect_1.encoding.hex.decode(parsed.args['payload']),
            });
        })
            .filter((vaa) => !!vaa);
    }
