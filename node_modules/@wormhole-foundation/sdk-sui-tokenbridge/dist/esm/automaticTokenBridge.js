...
import { encoding, isNative, serialize, toChainId, } from "@wormhole-foundation/sdk-connect";
import { getObjectFields, getPackageId, isMoveStructId, isMoveStructStruct, isSameType, SUI_COIN, SUI_SEPARATOR, SuiAddress, SuiPlatform, SuiUnsignedTransaction, } from "@wormhole-foundation/sdk-sui";
import "@wormhole-foundation/sdk-sui-core";
import { getTokenCoinType } from "./utils.js";
export class SuiAutomaticTokenBridge {
    network;
    chain;
    connection;
    contracts;
    ...
    async *transfer(sender, recipient, token, amount, nativeGas) {
        const tokenAddress = new SuiAddress(isNative(token) ? SuiPlatform.nativeTokenId(this.network, this.chain).address : token);
    ...
    async *redeem(sender, vaa) {
        ...
        const coinType = await getTokenCoinType(this.connection, this.tokenBridgeObjectId, tokenAddress.toUniversalAddress().toUint8Array(), toChainId(tokenChain));
        if (!coinType) {
    ...
    async maxSwapAmount(token) {
        ...
        return encoding.bignum.decode(new Uint8Array(result.results[0].returnValues[0][0].toReversed()));
    }
    ...
    async getRegisteredTokens() {
        ...
        const tokenAddresses = allTokensInfo.data.map((token) => {
            ...
            return new SuiAddress([address, module, name].join(SUI_SEPARATOR));
        });
        return tokenAddresses;
    }
    async isRegisteredToken(token) {
        const tokenAddress = new SuiAddress(isNative(token) ? SuiPlatform.nativeTokenId(this.network, this.chain).address : token).unwrap();
        try {
