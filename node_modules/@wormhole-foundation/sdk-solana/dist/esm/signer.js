...
export async function getWritableAccounts(connection, transaction) {
    if (isVersionedTransaction(transaction)) {
...
export async function determinePriorityFeeTritonOne(connection, transaction, percentile = DEFAULT_PRIORITY_FEE_PERCENTILE, multiple = DEFAULT_PERCENTILE_MULTIPLE, minPriorityFee = DEFAULT_MIN_PRIORITY_FEE, maxPriorityFee = DEFAULT_MAX_PRIORITY_FEE) {
    ...
    const unboundedFee = Math.floor(median(recentPrioritizationFees) * (multiple > 0 ? multiple : 1));
