...
import { join, resolve, extname, basename, relative } from 'pathe';
import { fileURLToPath, resolvePath as resolvePath$1, findExportNames, pathToFileURL } from 'mlly';
import { debounce } from 'perfect-debounce';
import MagicString from 'magic-string';
import { titleCase, camelCase } from 'scule';
import { readPackageJSON } from 'pkg-types';
import { defu } from 'defu';
import { link, image, md } from 'mdbox';
...
const jsimport = defineGenerator({
  ...
  async generate({ config, args }) {
    ...
    if (args.src) {
      const resolved = await resolvePath$1(args.src, { url: config.dir });
  ...
  async generate({ args, config, url }) {
    ...
    if (args.code) {
      contents = md.codeBlock(
        ...
        {
          // prettier-ignore
          ext: args.name === false ? void 0 : typeof args.name === "string" ? args.name : `[${basename(fullPath)}]`
        }
      );
    }
    return {
...
async function automd(_config = {}) {
  ...
  if (inputFiles.some((i) => i.includes("*"))) {
    ...
    inputFiles = inputFiles.map((i) => resolve(config.dir, i)).filter((i) => existsSync(i)).map((i) => relative(config.dir, i));
  }
