...
describe.concurrent('UTXO chains RPC check', () => {
    const rpcUrls = supportedUXTOChains.flatMap((chain) => chain.metamask.rpcUrls.map((rpcUrl) => ({
        rpcUrl: rpcUrl,
        chainId: chain.id,
...
describe.concurrent('UTXO chains block explorer check', () => {
    const blockExplorerUrls = supportedUXTOChains.flatMap((chain) => chain.metamask.blockExplorerUrls.map((blockExplorerUrl) => ({
