...
import { getApprovalAccount, getDistributionAccount, getTransferHookExtraAccounts, } from "../token2022";
...
export class TensorSwapSDK {
    ...
    // --------------------------------------- T22
    //main signature owner + payer
    async listT22({ nftMint, nftSource, owner, price, payer = null, compute = DEFAULT_XFER_COMPUTE_UNITS, priorityMicroLamports = DEFAULT_MICRO_LAMPORTS, transferHook = null, }) {
        const [tswapPda, tswapBump] = findTSwapPDA({});
        const [singleListing, singleListingBump] = findSingleListingPDA({
            nftMint,
        });
        ...
        if (transferHook) {
            const hookAccounts = await getTransferHookExtraAccounts(this.program.provider.connection, nftMint, await builder.instruction(), transferHook.program);
    ...
    // main signature: owner
    async delistT22({ nftMint, nftDest, owner, payer = null, compute = DEFAULT_XFER_COMPUTE_UNITS, priorityMicroLamports = DEFAULT_MICRO_LAMPORTS, transferHook = null, }) {
        const [tswapPda, tswapBump] = findTSwapPDA({});
        const [singleListing, singleListingBump] = findSingleListingPDA({
            nftMint,
        });
    ...
    //main signature: buyer
    async buySingleListingT22({ nftMint, nftBuyerAcc, owner, buyer, maxPrice, takerBroker = null, compute = DEFAULT_XFER_COMPUTE_UNITS, priorityMicroLamports = DEFAULT_MICRO_LAMPORTS, transferHook = null, }) {
        const [tswapPda, tswapBump] = findTSwapPDA({});
        const [singleListing, singleListingBump] = findSingleListingPDA({
            nftMint,
        });
    ...
    //main signature: buyer
    async buyNftT22({ whitelist, nftMint, nftBuyerAcc, owner, buyer, config, maxPrice, marginNr = null, takerBroker = null, compute = DEFAULT_XFER_COMPUTE_UNITS, priorityMicroLamports = DEFAULT_MICRO_LAMPORTS, transferHook = null, }) {
        const [tswapPda, tswapBump] = findTSwapPDA({});
        const [poolPda, poolBump] = findPoolPDA({
            tswap: tswapPda,
            owner,
    ...
    // main signature: owner
    async depositNftT22({ whitelist, nftMint, nftSource, owner, config, compute = DEFAULT_XFER_COMPUTE_UNITS, priorityMicroLamports = DEFAULT_MICRO_LAMPORTS, transferHook = null, }) {
        const [tswapPda, tswapBump] = findTSwapPDA({});
        const [poolPda, poolBump] = findPoolPDA({
            tswap: tswapPda,
            owner,
    ...
    //main signature: seller
    async sellNftT22({ type, whitelist, nftMint, nftSellerAcc, owner, seller, config, minPrice, marginNr = null, isCosigned = false, cosigner = TSWAP_COSIGNER, takerBroker = null, compute = DEFAULT_XFER_COMPUTE_UNITS, priorityMicroLamports = DEFAULT_MICRO_LAMPORTS, transferHook = null, }) {
        const [tswapPda, tswapBump] = findTSwapPDA({});
        const [poolPda, poolBump] = findPoolPDA({
            tswap: tswapPda,
            owner,
    ...
    // main signature: owner
    async withdrawNftT22({ whitelist, nftMint, nftDest, owner, config, compute = DEFAULT_XFER_COMPUTE_UNITS, priorityMicroLamports = DEFAULT_MICRO_LAMPORTS, transferHook = null, }) {
        const [tswapPda, tswapBump] = findTSwapPDA({});
        const [poolPda, poolBump] = findPoolPDA({
            tswap: tswapPda,
            owner,
