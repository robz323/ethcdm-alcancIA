import { CurveType, PoolType, TakerSide } from "../types";
import Big from "big.js";
import BN from "bn.js";
export const HUNDRED_PCT_BPS = 10000;
...
export const computeTakerDisplayPrice = (args) => {
...
slippage = config.curveType === CurveType.Linear ? 0 : EXPO_SLIPPAGE, marginated, }) => {
    // Cannot sell anymore into capped pool.
    if (takerSide === TakerSide.Sell &&
        marginated &&
        maxTakerSellCount != 0 &&
    ...
    if (basePrice.lt(0))
        return null;
    basePrice = basePrice.mul(1 + (takerSide === TakerSide.Buy ? 1 : -1) * slippage);
    return basePrice;
};
...
export const computeMakerAmountCount = ({ desired, maxCountWhenInfinite = 1000, ...priceArgs }) => {
    const currPriceArgs = {
        ...priceArgs,
        slippage: 0,
    };
    ...
    */
    ...
    const getTotalAmountLinear = (desiredCount) => {
        const allowedCount = adjustByMaxTakerCount(takerSide === TakerSide.Buy
            ? desiredCount
        ...
        const totalAmount = takerSide === TakerSide.Buy ? base.add(deltas) : base.minus(deltas);
        return {
            allowedCount,
    ...
    const getTotalAmountExp = (count) => {
        ...
        */
        ...
        return {
            allowedCount,
            // Negative slippage.
            totalAmount: new BN(adjustTotalMMFee(totalAmount)
                .mul(1 + EXPO_SLIPPAGE)
                .round()
                .toString()),
        };
