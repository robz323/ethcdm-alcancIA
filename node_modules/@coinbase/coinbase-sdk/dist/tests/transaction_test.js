...
describe("Transaction", () => {
    ...
    beforeEach(() => {
        ...
        transaction = new transaction_1.Transaction(model);
    });
    describe("constructor", () => {
        ...
        it("should raise an error when initialized with a model of a different type", () => {
            expect(() => new transaction_1.Transaction(null)).toThrow("Invalid model type");
        });
    });
    ...
    describe("#getSignedPayload", () => {
        ...
        it("should return the signed payload when the transaction has been broadcast on chain", () => {
            const transaction = new transaction_1.Transaction(broadcastedModel);
    ...
    describe("#getRawTransaction", () => {
        ...
        it("should return the raw transaction", () => {
            expect(raw).toBeInstanceOf(ethers_1.ethers.Transaction);
        });
    ...
    describe("#getTransactionLink", () => {
        it("should return the transaction link when the transaction hash is available", () => {
            const transaction = new transaction_1.Transaction(broadcastedModel);
            expect(transaction.getTransactionLink()).toEqual(`https://sepolia.basescan.org/tx/${transactionHash}`);
        });
    });
    describe("#sign", () => {
        let signature;
    ...
    describe("#getStatus", () => {
        [
            { status: types_1.TransactionStatus.PENDING, expected: "pending" },
            { status: types_1.TransactionStatus.BROADCAST, expected: "broadcast" },
            { status: types_1.TransactionStatus.SIGNED, expected: "signed" },
            { status: types_1.TransactionStatus.COMPLETE, expected: "complete" },
            { status: types_1.TransactionStatus.FAILED, expected: "failed" },
        ].forEach(({ status, expected }) => {
            describe(`when the status is ${status}`, () => {
                beforeEach(() => model.status = status);
                it(`should return ${expected}`, () => {
                    const transaction = new transaction_1.Transaction(model);
    ...
    describe("#blockHash", () => {
        it("returns the block hash", () => {
            const transaction = new transaction_1.Transaction(onchainModel);
    ...
    describe("#isTerminalState", () => {
        ...
        it("should be in a terminal state", () => {
            model.status = types_1.TransactionStatus.COMPLETE;
            const transaction = new transaction_1.Transaction(model);
            expect(transaction.isTerminalState()).toEqual(true);
        });
    ...
    describe("#toString", () => {
        ...
        it("returns the same value as toString", () => {
            const transaction = new transaction_1.Transaction(broadcastedModel);
            expect(transaction.toString()).toEqual(`Transaction { transactionHash: '${transaction.getTransactionHash()}', status: '${transaction.getStatus()}', unsignedPayload: '${transaction.getUnsignedPayload()}', signedPayload: ${transaction.getSignedPayload()}, transactionLink: ${transaction.getTransactionLink()} }`);
        });
