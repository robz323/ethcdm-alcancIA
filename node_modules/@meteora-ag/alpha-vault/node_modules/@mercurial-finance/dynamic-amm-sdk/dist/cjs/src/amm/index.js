...
const stake_for_fee_1 = __importStar(require("@meteora-ag/stake-for-fee"));
...
class AmmImpl {
    ...
    static async createPermissionlessConstantProductPoolWithConfig2(connection, payer, tokenAMint, tokenBMint, tokenAAmount, tokenBAmount, config, opt) {
        ...
        if (feeWrapperLockAmount.gt(new anchor_1.BN(0))) {
            ...
            const createFeeVaultIxs = await stake_for_fee_1.default.createFeeVaultInstructions(connection, poolPubkey, payer, tokenAMint, tokenBMint);
            preInstructions.push(...createFeeVaultIxs);
            const vaultKey = (0, stake_for_fee_1.deriveFeeVault)(poolPubkey, stake_for_fee_1.STAKE_FOR_FEE_PROGRAM_ID);
    ...
    static async createPermissionlessConstantProductMemecoinPoolWithConfig(connection, payer, tokenAMint, tokenBMint, tokenAAmount, tokenBAmount, config, memecoinInfo, opt) {
        const { vaultProgram, ammProgram } = (0, utils_1.createProgram)(connection, opt?.programId);
        const { tx: mintTx, mintAccount } = await (0, utils_1.createMint)(connection, memecoinInfo.keypair, memecoinInfo.payer, memecoinInfo.assetData, memecoinInfo.mintAuthority, null, memecoinInfo.decimals, spl_token_1.TOKEN_PROGRAM_ID);
        const createTokenIxs = [...mintTx.instructions];
        ...
        if (feeWrapperLockAmount.gt(new anchor_1.BN(0))) {
            ...
            const createFeeVaultIxs = await stake_for_fee_1.default.createFeeVaultInstructions(connection, poolPubkey, payer, tokenAMint, tokenBMint, initFeeVaultParams);
            preInstructions.push(...createFeeVaultIxs);
            const vaultKey = (0, stake_for_fee_1.deriveFeeVault)(poolPubkey, stake_for_fee_1.STAKE_FOR_FEE_PROGRAM_ID);
            const [lockEscrowFeeVaultPK] = (0, utils_1.deriveLockEscrowPda)(poolPubkey, vaultKey, ammProgram.programId);
            const [escrowFeeVaultAta, createEscrowFeeVaultAtaIx] = await (0, utils_1.getOrCreateATAInstruction)(lpMint, lockEscrowFeeVaultPK, connection, payer);
    ...
    async lockLiquidity(owner, amount, feePayer, opt) {
        ...
        if (feeWrapperLockAmount.gt(new anchor_1.BN(0))) {
            ...
            const vaultKey = (0, stake_for_fee_1.deriveFeeVault)(this.address, stake_for_fee_1.STAKE_FOR_FEE_PROGRAM_ID);
