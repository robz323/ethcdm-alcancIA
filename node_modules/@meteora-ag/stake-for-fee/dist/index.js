...
var computeUnitIx = (units) => {
  return _web3js.ComputeBudgetProgram.setComputeUnitLimit({
    units: _nullishCoalesce(units, () => ( 14e5))
  });
};

// src/stake-for-fee/constants/compute.ts
var COMPUTE_UNIT = Object.freeze({
  STAKE: 35e4,
  UNSTAKE: 25e4,
  CANCEL_UNSTAKE: 25e4,
  CLAIM: 25e4
...
var StakeForFee2 = class _StakeForFee {
  ...
  findLargestStakerNotInTopListFromFullBalanceList(lookupNumber) {
    ...
    const fullBalanceListLength = this.accountStates.fullBalanceListState.metadata.length.toNumber();
    const stakeBalanceSorter = (a, b) => {
    ...
    for (let i = 0; i < fullBalanceListLength; i++) {
      ...
      const largestStakerWithLowestPriority = largestStakers[largestStakers.length - 1];
  ...
  findReplaceableTopStaker(lookupNumber) {
    ...
    for (const staker of this.accountStates.topStakerListState.stakers) {
      ...
      const stakeBalanceAscSorted = (a, b) => {
        if (a.stakeAmount.eq(b.stakeAmount)) {
          return a.fullBalanceIndex.cmp(b.fullBalanceIndex);
        } else {
  ...
  async claimFee(owner, maxFee) {
    ...
    const claimTx = await this.stakeForFeeProgram.methods.claimFee(maxFee).accounts({
      userQuoteToken,
      vault: this.feeVaultKey,
      fullBalanceList: this.accountStates.feeVault.fullBalanceList,
      topStakerList: this.accountStates.feeVault.topStakerList,
