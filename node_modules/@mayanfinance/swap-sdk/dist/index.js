...
function getEvmMctpBridgeParams(quote, destinationAddress, signerChainId) {
  ...
  if (quote.toChain === "solana") {
    ...
    const [caller] = import_web33.PublicKey.findProgramAddressSync(
      [import_buffer3.Buffer.from("CALLER")],
      new import_web33.PublicKey(addresses_default.MCTP_PROGRAM_ID)
    );
    const [main] = import_web33.PublicKey.findProgramAddressSync(
      [import_buffer3.Buffer.from("MAIN")],
      new import_web33.PublicKey(addresses_default.MCTP_PROGRAM_ID)
    );
    const cctpOutputMint = new import_web33.PublicKey(quote.mctpOutputContract);
...
function getEvmMctpCreateOrderParams(quote, destinationAddress, referrerAddress, signerChainId) {
  ...
  const tokenOut = quote.toToken.contract === import_ethers2.ZeroAddress ? nativeAddressToHexString(import_web33.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.toToken.contract, quote.toToken.wChainId);
  return {
    params: {
      tokenIn: quote.mctpInputContract,
      amountIn,
...
function createSwiftOrderHash(quote, swapperAddress, destinationAddress, referrerAddress, randomKeyHex) {
  ...
  const data = import_buffer5.Buffer.alloc(orderDataSize);
  let offset = 0;
  const sourceChainId = getWormholeChainIdByName(quote.fromChain);
  const trader = import_buffer5.Buffer.from(hexToUint8Array(nativeAddressToHexString(swapperAddress, sourceChainId)));
  data.set(trader, 0);
  offset += 32;
  data.writeUInt16BE(sourceChainId, offset);
  offset += 2;
  const _tokenIn = quote.swiftInputContract === import_ethers3.ZeroAddress ? nativeAddressToHexString(import_web35.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.swiftInputContract, sourceChainId);
  const tokenIn = import_buffer5.Buffer.from(hexToUint8Array(_tokenIn));
  data.set(tokenIn, offset);
  offset += 32;
  const destinationChainId = getWormholeChainIdByName(quote.toChain);
  const destAddress = import_buffer5.Buffer.from(hexToUint8Array(nativeAddressToHexString(destinationAddress, destinationChainId)));
  data.set(destAddress, offset);
  offset += 32;
  data.writeUInt16BE(destinationChainId, offset);
  offset += 2;
  const _tokenOut = quote.toToken.contract === import_ethers3.ZeroAddress ? nativeAddressToHexString(import_web35.SystemProgram.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.toToken.contract, destinationChainId);
  const tokenOut = import_buffer5.Buffer.from(hexToUint8Array(_tokenOut));
  ...
  offset += 8;
  const refAddress = referrerAddress ? import_buffer5.Buffer.from(hexToUint8Array(nativeAddressToHexString(referrerAddress, destinationChainId))) : import_web35.SystemProgram.programId.toBuffer();
  ...
  offset += 1;
  const _randomKey = import_buffer5.Buffer.from(hexToUint8Array(randomKeyHex));
  ...
  return import_buffer5.Buffer.from(hexToUint8Array(hash));
}
var InitSwiftLayout = (0, import_buffer_layout2.struct)([
  (0, import_buffer_layout2.blob)(8, "instruction"),
  (0, import_buffer_layout2.blob)(8, "amountInMin"),
  (0, import_buffer_layout2.u8)("nativeInput"),
  (0, import_buffer_layout2.blob)(8, "feeSubmit"),
  ...
  (0, import_buffer_layout2.blob)(8, "feeCancel"),
  (0, import_buffer_layout2.blob)(8, "feeRefund"),
  (0, import_buffer_layout2.blob)(8, "deadline"),
  (0, import_buffer_layout2.blob)(32, "refAddress"),
  (0, import_buffer_layout2.u8)("feeRateRef"),
  (0, import_buffer_layout2.u8)("feeRateMayan"),
  (0, import_buffer_layout2.u8)("auctionMode"),
  (0, import_buffer_layout2.blob)(32, "randomKey")
]);
function createSwiftInitInstruction(params) {
  const { quote } = params;
  const mint = quote.swiftInputContract === import_ethers3.ZeroAddress ? solMint : new import_web35.PublicKey(quote.swiftInputContract);
  const destinationChainId = getWormholeChainIdByName(quote.toChain);
  ...
  const accounts = [
    ...
    { pubkey: new import_web35.PublicKey(addresses_default.FEE_MANAGER_PROGRAM_ID), isWritable: false, isSigner: false },
    { pubkey: new import_web35.PublicKey(addresses_default.TOKEN_PROGRAM_ID), isWritable: false, isSigner: false },
    { pubkey: import_web35.SystemProgram.programId, isWritable: false, isSigner: false }
  ];
  const data = import_buffer5.Buffer.alloc(InitSwiftLayout.span);
  const refAddress = params.referrerAddress ? import_buffer5.Buffer.from(hexToUint8Array(nativeAddressToHexString(params.referrerAddress, destinationChainId))) : import_web35.SystemProgram.programId.toBuffer();
  ...
  InitSwiftLayout.encode({
    ...
    destAddress: import_buffer5.Buffer.from(hexToUint8Array(nativeAddressToHexString(params.destinationAddress, destinationChainId))),
    destinationChain: destinationChainId,
    tokenOut: import_buffer5.Buffer.from(hexToUint8Array(nativeAddressToHexString(quote.toToken.contract, destinationChainId))),
  ...
  return new import_web35.TransactionInstruction({
    keys: accounts,
    data,
    programId: new import_web35.PublicKey(addresses_default.SWIFT_PROGRAM_ID)
  });
}
async function createSwiftFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  const swiftProgram = new import_web35.PublicKey(addresses_default.SWIFT_PROGRAM_ID);
  const trader = new import_web35.PublicKey(swapperAddress);
  const randomKey = import_web35.Keypair.generate();
  if (!Number(quote.deadline64)) {
  ...
  const [state] = import_web35.PublicKey.findProgramAddressSync(
    [import_buffer5.Buffer.from("STATE_SOURCE"), hash],
    swiftProgram
  );
  const stateAccount = getAssociatedTokenAddress(
    new import_web35.PublicKey(quote.swiftInputContract),
    state,
    true
  );
  const swiftInputMint = quote.swiftInputContract === import_ethers3.ZeroAddress ? solMint : new import_web35.PublicKey(quote.swiftInputContract);
  const relayer = quote.gasless ? new import_web35.PublicKey(quote.relayer) : trader;
  ...
  if (quote.fromToken.contract === quote.swiftInputContract) {
    if (quote.suggestedPriorityFee > 0) {
      instructions.push(import_web35.ComputeBudgetProgram.setComputeUnitPrice({
...
function getEvmSwiftParams(quote, swapperAddress, destinationAddress, referrerAddress, signerChainId) {
  ...
  if (referrerAddress) {
    ...
    referrerHex = nativeAddressToHexString(
      import_web36.SystemProgram.programId.toString(),
      1
    );
  }
  const random = nativeAddressToHexString(import_web36.Keypair.generate().publicKey.toString(), 1);
  const tokenOut = quote.toToken.contract === import_ethers4.ZeroAddress ? nativeAddressToHexString(import_web36.SystemProgram.programId.toString(), 1) : nativeAddressToHexString(quote.toToken.contract, destChainId);
  const minAmountOut = getAmountOfFractionalAmount(
    quote.minAmountOut,
    Math.min(8, quote.toToken.decimals)
  );
...
function getSwiftFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddress, signerChainId, permit) {
  ...
  signerChainId = Number(signerChainId);
  const _permit = permit || ZeroPermit;
  const forwarder = new import_ethers4.Contract(addresses_default.MAYAN_FORWARDER_CONTRACT, MayanForwarderArtifact_default.abi);
  const {
  ...
  let swiftCallData;
  const swiftContract = new import_ethers4.Contract(swiftContractAddress, MayanSwiftArtifact_default.abi);
...
function getEvmSwapParams(quote, destinationAddress, referrerAddress, signerAddress, signerChainId, payload) {
  const mayanProgram = new import_web37.PublicKey(addresses_default.MAYAN_PROGRAM_ID);
  const [mayanMainAccount] = import_web37.PublicKey.findProgramAddressSync(
    [import_buffer6.Buffer.from("MAIN")],
    mayanProgram
  );
  const recipient = getAssociatedTokenAddress(
    new import_web37.PublicKey(quote.fromToken.mint),
    mayanMainAccount,
    true
  );
  const amountIn = getAmountOfFractionalAmount(
  ...
  const criteria = {
    ...
    unwrap: unwrapRedeem,
    customPayload: payload ? `0x${import_buffer6.Buffer.from(payload).toString("hex")}` : "0x"
  };
...
function getSwapFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddresses, signerAddress, signerChainId, payload, permit) {
  ...
  const forwarderContract = new import_ethers5.Contract(addresses_default.MAYAN_FORWARDER_CONTRACT, MayanForwarderArtifact_default.abi);
  const mayanSwap = new import_ethers5.Contract(contractAddress, MayanSwapArtifact_default.abi);
  let forwarderMethod;
  let forwarderParams;
  let value;
  const _permit = permit || ZeroPermit;
  if (tokenIn === import_ethers5.ZeroAddress) {
    ...
    value = (0, import_ethers5.toBeHex)(bridgeFee);
  }
...
function getWormholePDAs(supplierProgram) {
  const wormholeProgramId = new import_web38.PublicKey(addresses_default.WORMHOLE_PROGRAM_ID);
  const programId = new import_web38.PublicKey(supplierProgram);
  const [bridgeConfig] = import_web38.PublicKey.findProgramAddressSync(
    [import_buffer7.Buffer.from("Bridge")],
    wormholeProgramId
  );
  const [emitter] = import_web38.PublicKey.findProgramAddressSync(
    [import_buffer7.Buffer.from("emitter")],
    programId
  );
  const [sequenceKey] = import_web38.PublicKey.findProgramAddressSync(
    [import_buffer7.Buffer.from("Sequence"), emitter.toBuffer()],
    wormholeProgramId
  );
  const [feeCollector] = import_web38.PublicKey.findProgramAddressSync(
    [import_buffer7.Buffer.from("fee_collector")],
    wormholeProgramId
  );
  return {
    bridgeConfig,
...
// src/solana/solanaMctp.ts
var MCTPBridgeWithFeeLayout = (0, import_buffer_layout3.struct)([
  (0, import_buffer_layout3.u8)("instruction"),
  (0, import_buffer_layout3.u32)("destinationDomain")
]);
function createMctpBridgeWithFeeInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  const wormholeProgramId = new import_web39.PublicKey(addresses_default.WORMHOLE_PROGRAM_ID);
  const TOKEN_PROGRAM_ID = new import_web39.PublicKey(addresses_default.TOKEN_PROGRAM_ID);
  const cctpCoreProgramId = new import_web39.PublicKey(addresses_default.CCTP_CORE_PROGRAM_ID);
  const cctpTokenProgramId = new import_web39.PublicKey(addresses_default.CCTP_TOKEN_PROGRAM_ID);
  const mctpProgram = new import_web39.PublicKey(addresses_default.MCTP_PROGRAM_ID);
  const relayer = new import_web39.PublicKey(relayerAddress);
  const mint = new import_web39.PublicKey(mintAddress);
  ...
  const cctpMessage = import_web39.Keypair.generate();
  const wormholeMessage = import_web39.Keypair.generate();
  const accounts = [
    ...
    { pubkey: import_web39.SYSVAR_CLOCK_PUBKEY, isWritable: false, isSigner: false },
    { pubkey: import_web39.SYSVAR_RENT_PUBKEY, isWritable: false, isSigner: false },
    ...
    { pubkey: import_web39.SystemProgram.programId, isWritable: false, isSigner: false },
    { pubkey: wormholeProgramId, isWritable: false, isSigner: false }
  ];
  const data = import_buffer8.Buffer.alloc(MCTPBridgeWithFeeLayout.span);
  ...
  const bridgeIns = new import_web39.TransactionInstruction({
    keys: accounts,
    data,
...
var MctpBridgeLockFeeLayout = (0, import_buffer_layout3.struct)([
  (0, import_buffer_layout3.u8)("instruction"),
  (0, import_buffer_layout3.u32)("destinationDomain")
]);
function createMctpBridgeLockFeeInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  ...
  const cctpBridgePdas = getCCTPBridgePDAs(mint, toChain);
  const cctpMessage = import_web39.Keypair.generate();
  const [feeState] = import_web39.PublicKey.findProgramAddressSync(
    [import_buffer8.Buffer.from("FEESTATE"), mint.toBuffer(), cctpMessage.publicKey.toBytes()],
    mctpProgram
  );
  let relayerAccount;
  if (feeSolana && feeSolana > BigInt(0)) {
  ...
  const data = import_buffer8.Buffer.alloc(MctpBridgeLockFeeLayout.span);
...
var MctpInitSwapLayout = (0, import_buffer_layout3.struct)([
  (0, import_buffer_layout3.u8)("instruction"),
  (0, import_buffer_layout3.u32)("destinationDomain")
]);
function createMctpInitSwapInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana) {
  ...
  const cctpTokenProgramId = new import_web39.PublicKey(addresses_default.CCTP_TOKEN_PROGRAM_ID);
  const mctpProgram = new import_web39.PublicKey(addresses_default.MCTP_PROGRAM_ID);
  const mayanSwapProgramId = new import_web39.PublicKey(addresses_default.MAYAN_PROGRAM_ID);
  const relayer = new import_web39.PublicKey(relayerAddress);
  const mint = new import_web39.PublicKey(mintAddress);
  const ledgerAccount = getAssociatedTokenAddress(
    mint,
  ...
  const cctpBridgePdas = getCCTPBridgePDAs(mint, toChain);
  const cctpMessage = import_web39.Keypair.generate();
  const [swapState] = import_web39.PublicKey.findProgramAddressSync(
    [import_buffer8.Buffer.from("SWAP"), ledger.toBuffer()],
    mctpProgram
  );
  const [mayanFee] = import_web39.PublicKey.findProgramAddressSync(
  ...
  const data = import_buffer8.Buffer.alloc(MctpInitSwapLayout.span);
  ...
  const initSwapIns = new import_web39.TransactionInstruction({
    keys: accounts,
    data,
    programId: mctpProgram
  });
...
var MctpBridgeLedgerLayout = (0, import_buffer_layout3.struct)([
  (0, import_buffer_layout3.u8)("instruction"),
  ...
  (0, import_buffer_layout3.blob)(8, "feeRedeem"),
  (0, import_buffer_layout3.blob)(8, "feeSolana"),
  (0, import_buffer_layout3.u32)("destinationDomain"),
  (0, import_buffer_layout3.u16)("destinationChain"),
  (0, import_buffer_layout3.u8)("mode")
]);
function createMctpBridgeLedgerInstruction(params) {
  ...
  const user = new import_web39.PublicKey(params.swapperAddress);
  const mint = new import_web39.PublicKey(params.mintAddress);
  ...
  const refAddress = params.referrerAddress ? import_buffer8.Buffer.from(hexToUint8Array(
    nativeAddressToHexString(params.referrerAddress, destinationChainId)
  )) : import_web39.SystemProgram.programId.toBuffer();
  const accounts = [
    ...
    { pubkey: import_web39.SYSVAR_RENT_PUBKEY, isWritable: false, isSigner: false },
    { pubkey: import_web39.SystemProgram.programId, isWritable: false, isSigner: false },
    { pubkey: new import_web39.PublicKey(refAddress), isWritable: false, isSigner: false }
  ];
  const data = import_buffer8.Buffer.alloc(MctpBridgeLedgerLayout.span);
  ...
  return new import_web39.TransactionInstruction({
    keys: accounts,
    data,
    programId: new import_web39.PublicKey(addresses_default.MCTP_PROGRAM_ID)
  });
}
var MctpSwapLedgerLayout = (0, import_buffer_layout3.struct)([
  (0, import_buffer_layout3.u8)("instruction"),
  (0, import_buffer_layout3.blob)(32, "destAddress"),
  (0, import_buffer_layout3.blob)(8, "amountInMin"),
  (0, import_buffer_layout3.blob)(8, "gasDrop"),
...
function createMctpSwapLedgerInstruction(params) {
  ...
  const feeSolana = getSafeU64Blob(params.feeSolana);
  const tokenOut = import_buffer8.Buffer.from(hexToUint8Array(
  ...
  const data = import_buffer8.Buffer.alloc(MctpSwapLedgerLayout.span);
...
async function createMctpFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  _lookupTablesAddress.push(addresses_default.LOOKUP_TABLE);
  const mctpProgram = new import_web39.PublicKey(addresses_default.MCTP_PROGRAM_ID);
  const user = new import_web39.PublicKey(swapperAddress);
  const randomKey = import_web39.Keypair.generate();
  ...
  const [ledger] = import_web39.PublicKey.findProgramAddressSync(
    [import_buffer8.Buffer.from("LEDGER"), user.toBytes(), randomKey.publicKey.toBytes()],
    mctpProgram
  );
  const ledgerAccount = getAssociatedTokenAddress(
    new import_web39.PublicKey(quote.mctpInputContract),
    ledger,
    true
  );
  ...
  if (quote.fromToken.contract === quote.mctpInputContract) {
    ...
    if (quote.suggestedPriorityFee > 0) {
      instructions.push(import_web39.ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: quote.suggestedPriorityFee
      }));
    }
    instructions.push(
      createAssociatedTokenAccountInstruction(user, ledgerAccount, ledger, new import_web39.PublicKey(quote.mctpInputContract))
    );
    instructions.push(
      createSplTransferInstruction(
        getAssociatedTokenAddress(
...
async function createSwapFromSolanaInstructions(quote, swapperWalletAddress, destinationAddress, referrerAddresses, connection, options = {}) {
  const referrerAddress = getQuoteSuitableReferrerAddress(quote, referrerAddresses);
  const solanaConnection = connection ?? new import_web310.Connection("https://rpc.ankr.com/solana");
  if (quote.type === "MCTP") {
  ...
  const tokenProgram = new import_web310.PublicKey(addresses_default.TOKEN_PROGRAM_ID);
  const swapper = new import_web310.PublicKey(swapperWalletAddress);
  const auctionAddr = new import_web310.PublicKey(addresses_default.AUCTION_PROGRAM_ID);
  if (quote.suggestedPriorityFee > 0) {
    instructions.push(import_web310.ComputeBudgetProgram.setComputeUnitPrice({
      microLamports: quote.suggestedPriorityFee
    }));
  }
  let referrerAddr;
  if (referrerAddress) {
    referrerAddr = new import_web310.PublicKey(referrerAddress);
  } else {
    referrerAddr = import_web310.SystemProgram.programId;
  }
  const [mayanFee, mayanFeeNonce] = import_web310.PublicKey.findProgramAddressSync(
    [import_buffer9.Buffer.from("MAYANFEE")],
    mayanProgram
  );
  const [referrerFee, referrerFeeNonce] = import_web310.PublicKey.findProgramAddressSync(
    [
      import_buffer9.Buffer.from("REFERRERFEE"),
      referrerAddr.toBuffer()
    ],
    mayanProgram
  );
  const msg1 = import_web310.Keypair.generate();
  const msg2 = import_web310.Keypair.generate();
  const [state, stateNonce] = import_web310.PublicKey.findProgramAddressSync(
    [
      import_buffer9.Buffer.from("V2STATE"),
      import_buffer9.Buffer.from(msg1.publicKey.toBytes()),
      import_buffer9.Buffer.from(msg2.publicKey.toBytes())
    ],
    mayanProgram
  );
  const fromMint = new import_web310.PublicKey(quote.fromToken.mint);
  const toMint = new import_web310.PublicKey(quote.toToken.mint);
  ...
  if (quote.fromToken.contract === import_ethers6.ZeroAddress) {
    instructions.push(import_web310.SystemProgram.transfer({
      fromPubkey: swapper,
  ...
  const delegate = import_web310.Keypair.generate();
  ...
  const swapKeys = [
    ...
    { pubkey: import_web310.SYSVAR_CLOCK_PUBKEY, isWritable: false, isSigner: false },
    { pubkey: import_web310.SYSVAR_RENT_PUBKEY, isWritable: false, isSigner: false },
    { pubkey: tokenProgram, isWritable: false, isSigner: false },
    { pubkey: import_web310.SystemProgram.programId, isWritable: false, isSigner: false }
  ];
  const destinationChainId = getWormholeChainIdByName(quote.toChain);
  if (destinationChainId === 1) {
    const destinationAccount = await solanaConnection.getAccountInfo(new import_web310.PublicKey(destinationAddress));
  ...
  const unwrapRedeem = quote.toToken.contract === import_ethers6.ZeroAddress;
  const unwrapRefund = quote.fromToken.contract === import_ethers6.ZeroAddress;
  if (!Number(quote.deadline64)) {
    throw new Error("Deadline is not valid");
  }
  const deadline = BigInt(quote.deadline64);
  const swapData = import_buffer9.Buffer.alloc(SwapLayout.span);
  const swapFields = {
    instruction: 101,
    stateNonce,
    amount: getSafeU64Blob(amount),
  ...
  const swapInstruction = new import_web310.TransactionInstruction({
    keys: swapKeys,
    data: swapData,
...
async function swapFromSolana(quote, swapperWalletAddress, destinationAddress, referrerAddresses, signTransaction, connection, extraRpcs, sendOptions, jitoOptions, instructionOptions) {
  ...
  const swapper = new import_web310.PublicKey(swapperWalletAddress);
  const feePayer = quote.gasless ? new import_web310.PublicKey(quote.relayer) : swapper;
  const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash();
  const message = import_web310.MessageV0.compile({
    instructions,
    payerKey: feePayer,
    recentBlockhash: blockhash,
    addressLookupTableAccounts: lookupTables
  });
  const transaction = new import_web310.VersionedTransaction(message);
  ...
  if (quote.gasless) {
    const serializedTrx = import_buffer9.Buffer.from(signedTrx.serialize()).toString("base64");
